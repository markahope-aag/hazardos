-- ============================================
-- REPAIR MIGRATION: recreate missing tables
-- ============================================
-- The migration history table (supabase_migrations.schema_migrations)
-- claims these migrations are applied, but 37 tables they define are
-- missing from the database. This migration replays the DDL with
-- idempotent guards (CREATE TABLE IF NOT EXISTS, DROP POLICY IF EXISTS,
-- etc.) so it is a no-op on healthy environments and a fix on drifted
-- ones.
--
-- The sources below are replayed verbatim except for idempotency
-- transforms and guarded seed data.

-- ============================================
-- SOURCE: 20260201000010_jobs_system.sql
-- ============================================
-- ============================================
-- JOBS TABLE
-- ============================================
...
-- ============================================
-- SOURCE: 20260201000020_invoices.sql
-- ============================================
-- ============================================
-- INVOICES SYSTEM
-- ============================================
...
-- ============================================
-- SOURCE: 20260201000025_organization_integrations.sql
-- ============================================
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_customers_qb ON customers(qb_customer_id) WHERE qb_customer_id IS NOT NULL;
...
CREATE POLICY "Users can manage their org integrations" ON organization_integrations FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "Users can view their org sync logs" ON integration_sync_log;


CREATE POLICY "Users can view their org sync logs" ON integration_sync_log FOR SELECT
  USING (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "System can insert sync logs" ON integration_sync_log;


CREATE POLICY "System can insert sync logs" ON integration_sync_log FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());

-- ============================================
-- SOURCE: 20260202000001_customer_contacts.sql
-- ============================================
...
CREATE TABLE IF NOT EXISTS customer_contacts (
  ...
  -- The original migration also declared a composite FK
  -- FOREIGN KEY (organization_id, customer_id) REFERENCES
  -- customers(organization_id, id). That requires a UNIQUE(org_id, id) on
  -- customers which isn't present here, so we rely on the single-column
  -- FK on customer_id above — it's sufficient for referential integrity.
  CONSTRAINT customer_contacts_id_unique UNIQUE (id)
);
...
CREATE INDEX IF NOT EXISTS idx_customer_contacts_role ON customer_contacts(customer_id, role);
...
CREATE POLICY "Users can view contacts in their organization" ON customer_contacts FOR SELECT
  USING (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can insert contacts in their organization" ON customer_contacts FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can update contacts in their organization" ON customer_contacts FOR UPDATE
  USING (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can delete contacts in their organization" ON customer_contacts FOR DELETE
  USING (organization_id = get_user_organization_id());
...
DROP TRIGGER IF EXISTS sync_primary_contact_trigger ON customer_contacts;
...
DROP TRIGGER IF EXISTS ensure_primary_contact_trigger ON customer_contacts;
...
-- ============================================
-- SOURCE: 20260203000001_platform_billing.sql
-- ============================================
-- ============================================
...
CREATE POLICY "Users can view their org subscription" ON organization_subscriptions FOR SELECT
  USING (organization_id = get_user_organization_id());

-- Invoices - org members can view their own
DROP POLICY IF EXISTS "Users can view their org invoices" ON billing_invoices;

CREATE POLICY "Users can view their org invoices" ON billing_invoices FOR SELECT
  USING (organization_id = get_user_organization_id());

-- Payment methods - org admins only
DROP POLICY IF EXISTS "Admins can view payment methods" ON payment_methods;

CREATE POLICY "Admins can view payment methods" ON payment_methods FOR SELECT
  USING (organization_id = get_user_organization_id());
...
DROP TRIGGER IF EXISTS update_users_count_on_profile_change ON profiles;
...
DROP TRIGGER IF EXISTS set_organization_subscriptions_updated_at ON organization_subscriptions;
...
-- ============================================
-- SOURCE: 20260215000001_job_completion.sql
-- ============================================
-- ============================================
...
-- ============================================
-- SOURCE: 20260215000002_customer_feedback.sql
-- ============================================
-- ============================================
...
-- ============================================
-- SOURCE: 20260220000001_reporting.sql
-- ============================================
-- ============================================
...
-- ============================================
-- Skipped in this repair migration: these views were written against a
-- schema that has since drifted (proposals.total, jobs.title,
-- jobs.completed_at, customers.source, etc. no longer match). They can
-- be rebuilt targeting the current schema when reporting is needed.
...
WHERE p.role IN ('admin', 'estimator', 'tenant_owner')
...
DROP POLICY IF EXISTS "Users can view own and shared reports" ON saved_reports;

CREATE POLICY "Users can view own and shared reports" ON saved_reports FOR SELECT
  USING (organization_id = get_user_organization_id()
...
CREATE POLICY "Users can manage own reports" ON saved_reports FOR ALL
  USING (organization_id = get_user_organization_id() AND created_by = auth.uid());

-- Users can view org exports
DROP POLICY IF EXISTS "Users can view org exports" ON report_exports;

CREATE POLICY "Users can view org exports" ON report_exports FOR SELECT
  USING (organization_id = get_user_organization_id());

-- Users can create exports
DROP POLICY IF EXISTS "Users can create exports" ON report_exports;

CREATE POLICY "Users can create exports" ON report_exports FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());

-- ============================================
-- UPDATED_AT TRIGGERS
-- ============================================
DROP TRIGGER IF EXISTS set_saved_reports_updated_at ON saved_reports;
...
-- ============================================
-- SOURCE: 20260220000002_sales_tools.sql
-- ============================================
-- ============================================
-- SALES TOOLS
...
DROP POLICY IF EXISTS "Users see own commissions" ON commission_earnings;

CREATE POLICY "Users see own commissions" ON commission_earnings FOR SELECT
  USING (organization_id = get_user_organization_id()
    AND (user_id = auth.uid() OR EXISTS (
      SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'tenant_owner')
    )));

DROP POLICY IF EXISTS "Admins manage commissions" ON commission_earnings;
...
DROP POLICY IF EXISTS "Admins manage approval_thresholds" ON approval_thresholds;
...
DROP POLICY IF EXISTS "Users view approval_thresholds" ON approval_thresholds;
...
DROP POLICY IF EXISTS "Org access approval_requests" ON approval_requests;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'create_pipeline_stages_for_new_org') THEN
    DROP TRIGGER IF EXISTS create_pipeline_stages_for_new_org ON organizations;
...
-- ============================================
-- SOURCE: 20260301000002_calendar_automation.sql
-- ============================================
-- ============================================
...
-- ============================================
CREATE TABLE IF NOT EXISTS lead_webhook_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- INDEXES
-- ============================================
CREATE INDEX IF NOT EXISTS idx_calendar_sync_org ON calendar_sync_events(organization_id);
CREATE INDEX IF NOT EXISTS idx_calendar_sync_job ON calendar_sync_events(job_id);
CREATE INDEX IF NOT EXISTS idx_calendar_sync_google ON calendar_sync_events(google_event_id) WHERE google_event_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_calendar_sync_outlook ON calendar_sync_events(outlook_event_id) WHERE outlook_event_id IS NOT NULL;
...
CREATE INDEX IF NOT EXISTS idx_lead_endpoints_org ON lead_webhook_endpoints(organization_id);
CREATE INDEX IF NOT EXISTS idx_lead_endpoints_slug ON lead_webhook_endpoints(slug);

CREATE INDEX IF NOT EXISTS idx_lead_log_endpoint ON lead_webhook_log(endpoint_id);
CREATE INDEX IF NOT EXISTS idx_lead_log_status ON lead_webhook_log(status);
...
CREATE POLICY "Users can manage their org calendar events" ON calendar_sync_events FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can manage their org webhooks" ON webhooks FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "System can manage webhook deliveries" ON webhook_deliveries FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "Users can manage their org lead endpoints" ON lead_webhook_endpoints;


CREATE POLICY "Users can manage their org lead endpoints" ON lead_webhook_endpoints FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "Users can view their org lead logs" ON lead_webhook_log;


CREATE POLICY "Users can view their org lead logs" ON lead_webhook_log FOR SELECT
  USING (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "System can insert lead logs" ON lead_webhook_log;


CREATE POLICY "System can insert lead logs" ON lead_webhook_log FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());

-- ============================================
-- TRIGGERS
-- ============================================
DROP TRIGGER IF EXISTS update_calendar_sync_events_updated_at ON calendar_sync_events;
...
DROP TRIGGER IF EXISTS update_lead_endpoints_updated_at ON lead_webhook_endpoints;
...
-- ============================================
-- SOURCE: 20260301000003_platform_extensibility.sql
-- ============================================
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_api_request_log_org ON api_request_log(organization_id);
...
CREATE INDEX IF NOT EXISTS idx_location_users_location ON location_users(location_id);
CREATE INDEX IF NOT EXISTS idx_location_users_user ON location_users(user_id);

CREATE INDEX IF NOT EXISTS idx_jobs_location ON jobs(location_id) WHERE location_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_site_surveys_location ON site_surveys(location_id) WHERE location_id IS NOT NULL;
...
CREATE POLICY "Users can manage their org API keys" ON api_keys FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "Users can view their org API request logs" ON api_request_log;


CREATE POLICY "Users can view their org API request logs" ON api_request_log FOR SELECT
  USING (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "System can insert API request logs" ON api_request_log;


CREATE POLICY "System can insert API request logs" ON api_request_log FOR INSERT
  WITH CHECK (organization_id = get_user_organization_id());

DROP POLICY IF EXISTS "Users can manage their org custom domains" ON custom_domains;


CREATE POLICY "Users can manage their org custom domains" ON custom_domains FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can manage their org locations" ON locations FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can manage location user assignments for their org" ON location_users FOR ALL
  USING (
    location_id IN (
...
-- ============================================
-- SOURCE: 20260301000004_ai_features.sql
-- ============================================
-- ============================================
-- PHASE 7: AI FEATURES
...
-- ============================================
CREATE TABLE IF NOT EXISTS estimate_suggestions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
...
-- INDEXES
-- ============================================
CREATE INDEX IF NOT EXISTS idx_estimate_suggestions_org ON estimate_suggestions(organization_id);
CREATE INDEX IF NOT EXISTS idx_estimate_suggestions_survey ON estimate_suggestions(site_survey_id) WHERE site_survey_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_photo_analyses_org ON photo_analyses(organization_id);
CREATE INDEX IF NOT EXISTS idx_photo_analyses_hash ON photo_analyses(image_hash);
CREATE INDEX IF NOT EXISTS idx_photo_analyses_job_photo ON photo_analyses(job_photo_id) WHERE job_photo_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_voice_transcriptions_org ON voice_transcriptions(organization_id);
CREATE INDEX IF NOT EXISTS idx_voice_transcriptions_user ON voice_transcriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_voice_transcriptions_context ON voice_transcriptions(context_type, context_id);
...
CREATE POLICY "Users can manage their org estimate suggestions" ON estimate_suggestions FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can manage their org photo analyses" ON photo_analyses FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
...
CREATE POLICY "Users can manage their org voice transcriptions" ON voice_transcriptions FOR ALL
  USING (organization_id = get_user_organization_id())
  WITH CHECK (organization_id = get_user_organization_id());
