-- ============================================
-- Diagnostic: re-check missing tables post-repair
-- ============================================
DO $$
DECLARE
  expected TEXT[] := ARRAY[
...
BEGIN
  ...
  IF array_length(missing, 1) IS NULL THEN
    RAISE NOTICE 'TABLE_AUDIT_POST: all % expected tables present', array_length(expected, 1);
  ELSE
    RAISE NOTICE 'TABLE_AUDIT_POST: % of % tables still missing: %',
