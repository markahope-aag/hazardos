    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded-full" style={{ backgroundColor: opportunity.stage?.color || '#6366f1' }} />
                  <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{opportunity.expected_close_date ? new Date(opportunity.expected_close_date).toLocaleDateString() : 'Not set'}</p>
                  </div>
                </div>
              </div>
              {opportunity.loss_reason && (
                <>
                  ...
                  <div>
                    ...
                    {opportunity.loss_notes && <p className="mt-2 text-sm">{opportunity.loss_notes}</p>}
                    {opportunity.competitor && <p className="mt-1 text-sm text-muted-foreground">Competitor: {opportunity.competitor}</p>}
                  </div>
                </>
              )}
            </CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><History className="h-5 w-5" />Stage History</CardTitle>
            </CardHeader>
            <CardContent>
              {history.length === 0 ? (
                  ...
                  {history.map((entry, idx) => (
                    <div key={entry.id} className="flex items-start gap-3">
                      <div className="relative">
                        <div className="w-3 h-3 rounded-full mt-1" style={{ backgroundColor: entry.to_stage?.color || '#6366f1' }} />
                        {idx < history.length - 1 && <div className="absolute top-4 left-1.5 w-0.5 h-full -ml-px bg-border" />}
                      </div>
                      <div className="flex-1 pb-4">
                        <span className="font-medium">
                          {entry.from_stage ? <>{entry.from_stage.name} &rarr; {entry.to_stage?.name}</> : <>Created in {entry.to_stage?.name}</>}
                        </span>
                        <p className="text-sm text-muted-foreground">
                          {entry.changed_by_user?.full_name || 'Unknown'} &bull; {new Date(entry.created_at).toLocaleString()}
                        </p>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><DollarSign className="h-5 w-5" />Value</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div>
                <p className="text-sm text-muted-foreground">Estimated Value</p>
                <p className="text-2xl font-bold">{formatCurrency(opportunity.estimated_value || 0, false)}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Weighted Value</p>
                <p className="text-xl font-semibold text-primary">{formatCurrency(opportunity.weighted_value || 0, false)}</p>
                <p className="text-xs text-muted-foreground">Based on {opportunity.stage?.probability || 0}% probability</p>
              </div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Building className="h-5 w-5" />Contact</CardTitle></CardHeader>
            <CardContent>
              <p className="font-medium">{customerName}</p>
              {opportunity.customer?.id && (
                <Button variant="link" asChild className="p-0 h-auto mt-2">
                  <Link href={`/crm/contacts/${opportunity.customer.id}`}>View Contact &rarr;</Link>
                </Button>
              )}
            </CardContent>
          </Card>
              ...
              <CardContent className="space-y-2">
                {opportunity.estimate_id && (
                  <Button variant="outline" asChild className="w-full justify-start">
                    <Link href={`/estimates/${opportunity.estimate_id}`}>View Estimate &rarr;</Link>
                  </Button>
                )}
                {opportunity.job_id && (
                  <Button variant="outline" asChild className="w-full justify-start">
                    <Link href={`/jobs/${opportunity.job_id}`}>View Job &rarr;</Link>
                  </Button>
                )}
              </CardContent>
            </Card>
