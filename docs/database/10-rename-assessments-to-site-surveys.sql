...
ALTER TABLE site_survey_photos 
  DROP CONSTRAINT assessment_photos_assessment_id_fkey;
...
CREATE INDEX idx_site_survey_photos_site_survey_id ON site_survey_photos(site_survey_id);
CREATE INDEX idx_site_survey_photos_created_at ON site_survey_photos(created_at);
CREATE INDEX idx_site_surveys_organization_id ON site_surveys(organization_id);
...
-- Step 9: Update any existing data references (if needed)
-- This would update any existing records, but since we're changing the schema,
-- the application code will need to be updated to use the new table names

-- Step 10: Add comments to document the change
...
-- Step 11: Update storage bucket policies (these need to be updated in Storage Dashboard)
/*
Note: The following storage policies need to be updated in Supabase Dashboard > Storage > assessment-media > Policies:

1. Rename bucket from 'assessment-media' to 'site-survey-media' (optional, or keep existing name)
2. Update policy descriptions to reference "site survey" instead of "assessment"

The storage policies will continue to work as-is since they're based on organization folders,
but you may want to update the policy names and descriptions for consistency.
*/
...
FROM site_surveys s
LEFT JOIN site_survey_photos p ON s.id = p.site_survey_id
GROUP BY s.id, s.job_name
LIMIT 5;